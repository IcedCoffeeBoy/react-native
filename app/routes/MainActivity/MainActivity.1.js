...
export default class MainActivity extends Component {
  ...
  _loading = async () => {
    ...
    await firebase.database().ref(eventPath).orderByKey().once('value').then(
      (eventData) => {
        eventData.forEach((eventChild) => {
          this.getEventIcon(eventChild.key)
            .then((uriObj) => {
              ...
              if (uriObj === 0) {
                eventObject.eventIconSrc = eventDefaultIcon;
              } else {
                eventObject.eventIconSrc = uriObj;
              }

              if (this.searchId(eventChild.key)) {
                this.setState({ data: [...this.state.data, eventObject] })
              } else {
                this.setState({ dataUser: [...this.state.dataUser, eventObject] })
              }
            })
        })
      }
  ...
  async _refresh() {
    await this.setState({ data: [], dataUser: [] })
    this.data = []
    this.dataUser = []
